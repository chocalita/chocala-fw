 ...
 * @author ypra
 */
class FrontController implements HttpManagement, Singleton
{
    ...
    private function regularRouting(GlobalVars $gvr)
    {
        try{
            ...
            if(ChocalaVars::asBoolean(Config::_('app.run.modular'))){
                $module = lcfirst(trim($gvr->module()!=''?
                        str_replace('-', '.', $gvr->module()):
                        Config::_('app.default.module')));
                $modulePath.= $module;
                $this->classPath = $modulePath.'.';
    ...
    public function genericRouting(GlobalVars $gvr)
    {
        try {
            $domain = trim($gvr->controller() != '' ? $gvr->controller() :
                Config::_('app.default.controller'));
            ...
            if (!Chocala::exist(Chocala::namespacePath($controllerDir))) {
                ...
                throw new ChocalaException(ChocalaErrors::CLASS_NOT_FOUND, 1);
            } else {
                Chocala::import($controllerPath);
                $action = lcfirst(trim($gvr->action() != '' ? $gvr->action() :
                    Config::_('app.default.action')));
                if (Chocala::classImplements($class, 'Chocala\Base\IController')) {
            ...
            ChocalaErrorsManager::manage($che);
            if (strtoupper(Config::_('app.run.environment')) != 'DEVELOPMENT') {
...
//                exit();
                ...
                if (ChocalaVars::asBoolean(Config::_('app.run.modular')) &&
                    $this->module == null) {
                    $this->module = Config::_('app.default.module');
                }
                if ($this->controller == null) {
                    $this->module = ChocalaVars::asBoolean(
                        Config::_('app.run.modular')) ?
                        Config::_('app.default.module') : null;
    ...
    public function noRouted()
    {
        ...
        if($this->action == null){
            $this->action = lcfirst(Config::_('app.default.404'));
        }
    }
