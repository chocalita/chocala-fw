...
/**
 * This class defines the structure of the 'job_area' table.
 *
 *
 *
 ...
 *
 */
class JobAreaTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = '.Map.JobAreaTableMap';
     ...
     */
    const TABLE_NAME = 'job_area';

    /**
     * The related Propel class for this table
     */
    const OM_CLASS = '\\JobArea';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'JobArea';
     ...
     */
    const COL_ID = 'job_area.ID';

    /**
     * the column name for the CODIGO field
     */
    const COL_CODIGO = 'job_area.CODIGO';
     ...
     */
    const COL_DESCRIPCION = 'job_area.DESCRIPCION';

    /**
     * the column name for the STATUS field
     */
    const COL_STATUS = 'job_area.STATUS';

    /**
     * the column name for the LAST_USER_ID field
     */
    const COL_LAST_USER_ID = 'job_area.LAST_USER_ID';

    /**
     * the column name for the CREATION_DATE field
     */
    const COL_CREATION_DATE = 'job_area.CREATION_DATE';

    /**
     * the column name for the MODIFICATION_DATE field
     */
    const COL_MODIFICATION_DATE = 'job_area.MODIFICATION_DATE';
    ...
    {
        // attributes
        $this->setName('job_area');
        $this->setPhpName('JobArea');
        $this->setIdentifierQuoting(false);
        $this->setClassName('\\JobArea');
        $this->setPackage('');
  ...
  array (
    ...
    {
        return $withPrefix ? JobAreaTableMap::CLASS_DEFAULT : JobAreaTableMap::OM_CLASS;
    }
     ...
     * @return array           (JobArea object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = JobAreaTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = JobAreaTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + JobAreaTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = JobAreaTableMap::OM_CLASS;
            /** @var JobArea $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            JobAreaTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = JobAreaTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = JobAreaTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    public static function addSelectColumns(Criteria $criteria, $alias = null)
    {
        if (null === $alias) {
            ...
            $criteria->addSelectColumn(JobAreaTableMap::COL_CREATION_DATE);
            $criteria->addSelectColumn(JobAreaTableMap::COL_MODIFICATION_DATE);
        } else {
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(JobAreaTableMap::DATABASE_NAME)->getTable(JobAreaTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(JobAreaTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(JobAreaTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new JobAreaTableMap());
        }
    }

    /**
     * Performs a DELETE on the database, given a JobArea or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or JobArea object or primary key or array of primary keys
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \JobArea) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(JobAreaTableMap::DATABASE_NAME);
            $criteria->add(JobAreaTableMap::COL_ID, (array) $values, Criteria::IN);
        }

        $query = JobAreaQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) {
            JobAreaTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) {
                JobAreaTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return JobAreaQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a JobArea or Criteria object.
     *
     * @param mixed               $criteria Criteria or JobArea object containing data that is used to create the INSERT statement.
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from JobArea object
        }

        if ($criteria->containsKey(JobAreaTableMap::COL_ID) && $criteria->keyContainsValue(JobAreaTableMap::COL_ID) ) {
